        ...
        <div className="w-full mb-20 flex justify-end">
            <div className='flex gap-2 p-1 items-center'>
                <div className="flex gap-2 text-sm font-sans align-middle items-center py-1 px-3 rounded-full border border-gray-700">
                    ...
                    <h2 className="text-sm text-gray-700k">{recruiterName}</h2>
                </div>
                <div id="hoverCard" onClick={handleInfoOnClick} onMouseEnter={() => { showInfo == "hidden" ? setShowInfo("hover") : setShowInfo("tap") }} onMouseLeave={() => { showInfo == "tap" ? setShowInfo("tap") : setShowInfo("hidden") }} className='hover:ring-2 hover:ring-gray-300 rounded-full px-[9px] py-[3px] cursor-pointer text-sm bg-gray-400 text-white'>{recruiterName.charAt(0).toUpperCase()}</div>
            </div>
            {(showInfo == "tap" || showInfo == "hover") && (
                <div className={`absolute top-14`} id="infoCard">
                    <div className="bg-white shadow-md rounded-md p-2">
                        <div>
                            <p className="text-gray-600 text-sm">{recruiterName}</p>
                            <p className="text-gray-600 text-sm">{recruiterEmail}</p>
                            <hr className='mt-2' />
