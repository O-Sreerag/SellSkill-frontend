...
function Card({ interview, isHovered, onMouseEnter, onMouseLeave, isSelected, onSelect }: {
    ...
    const handleConform = async (e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        try {
            const response = await api.put(`/interview/update/${interview._id}`, { conformStatusOnApplicant: "accepted" })
            console.log(response.data)

            if (response.status == 200) {
                // Create notification data
                const notificationData1 = {
                    type: NotificationType.InterviewConfirmed,
                    userId: response.data?.result?.recruiterId,
                    role: 'applicant',
                    message: `Interview has been confirmed by applicant "${userName}" has been created!`,
