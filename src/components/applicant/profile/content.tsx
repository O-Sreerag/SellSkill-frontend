import { useEffect, useState } from "react";
import { useSelector } from "react-redux";

import { AppRootState } from "../../../redux/store";
import Mainbody from "./Mainbody"
import Sidebar from "../Sidebar"
import Navbar2 from "../Navbar2"

import Chat from '../../messages/content'
import Notification from '../../notifications/content'

const Content = () => {
    const [activePage, setActivePage] = useState('profile');
    const { activeMenu } = useSelector((state: AppRootState) => state.active);
    const [sidebarStyle, setSidebarStyle] = useState<{ transform?: string, transition?: string }>({});

    useEffect(() => {
        if (activeMenu) {
            setSidebarStyle({
                transform: 'translateX(0)',
                transition: 'transform 0.3s ease-in-out',
            });
        } else {
            setSidebarStyle({
                transform: 'translateX(-400px)',
                transition: 'transform 0.3s ease-in-out',
            });
        }
    }, [activeMenu]);

    return (
        <div>
            <div className="flex min-h-screen flex-auto flex-shrink-0 bg-[#f8fafa] text-gray-800 antialiased">
                <div className="w-60 relative">
                    <Sidebar activePage={activePage} setActivePage={setActivePage} />
                    <div
                        className={`fixed ${activeMenu ? 'left-[240px]' : '-left-[240px]'} bg-[#f8fafa] border-r border-gray-200 shadow-md w-[400px] h-screen`}
                        style={{ ...sidebarStyle, }}>
                        {activeMenu === "messages" ? <Chat role="applicant" /> : <Notification role="applicant" />}
                    </div>
                </div>

                <div className="flex justify-center w-full bg-[#f8fafa]">
                    <div className='max-w-[1100px] lg:min-w-[1100px]'>
                        <div className="items-center flex flex-col justify-between p-5">
                            <Navbar2 />
                            <div className="w-full space-y-1 mb-5">
                                <h2 className="text-2xl sm:text-3xl md:text-4xl text-[#2B2B2E] justify-start barlow-regular">Profile Management</h2>
                                <p className="text-gray-600">View and Edit your Profile.</p>
                            </div>
                            <Mainbody />
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Content